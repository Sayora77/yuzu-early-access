...
void GMainWindow::UpdateWindowTitle(const QString& title_name) {
    ...
    if (title_name.isEmpty()) {
        ...
        setWindowTitle(QString::fromStdString(fmt::format(fmt.empty() ? "yuzu Early Access 80" : fmt,
                                                          full_name, branch_name, description,
                                                          std::string{}, date, build_id)));
    } else {
        const auto fmt = std::string(Common::g_title_bar_format_running);
        setWindowTitle(QString::fromStdString(
            fmt::format(fmt.empty() ? "yuzu Early Access 80 {0}| {3}" : fmt, full_name, branch_name,
